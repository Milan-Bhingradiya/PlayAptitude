...
export function ResultsPage() {
  ...
  const winner = results.user.score > results.opponent.score ? "user" : "opponent"
      ...
      <Card className="bg-gray-800/50 shadow-xl w-full max-w-3xl backdrop-blur-md">
        <CardContent className="p-8">
          ...
          <div className="grid grid-cols-2 gap-12 mb-12">
            {["user", "opponent"].map((player) => (
              <div key={player} className="text-center relative">
                <div className={`absolute inset-0 bg-blue-500 opacity-10 rounded-full filter blur-2xl ${winner === player ? 'animate-pulse' : ''}`}></div>
                <Avatar className="w-32 h-32 mx-auto mb-6 relative ring-4 ring-offset-4 ring-offset-gray-800 ring-blue-500">
                  <AvatarImage src={results[player].avatar} alt={results[player].name} />
                ...
                <h3 className={`text-2xl font-bold mb-2 ${winner === player ? 'text-yellow-400' : ''}`}>{results[player].name}</h3>
                <p className="text-3xl font-bold text-blue-300 mb-3">{results[player].score} points</p>
                <p className="text-sm mb-2">{results[player].correctAnswers} / 10 correct</p>
                <Progress 
          ...
          <div className="flex justify-center space-x-6">
            <Button 
              className="bg-blue-600 hover:bg-blue-700 text-lg px-8 py-6 rounded-full transition-all duration-300 ease-in-out transform hover:scale-105"
            >
